...
|------|--------|----------|
| Treasury keys via AWS Secrets Manager + KMS CMK | Roadmap | Replace Lambda env-var keys with `secretsmanager:GetSecretValue` at cold-start; KMS CMK with separate IAM for read vs write; auto-rotation policy. |
